...
function HPBar({ name, level, index, franchise }: { name: string; level: number; index: number; franchise: Franchise }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-1 h-3 border overflow-hidden relative ${franchise === "pokemon" ? "rounded-full" : franchise === "overwatch" ? "rounded-[2px]" : "rounded-sm"}`}
        style={{
      ...
      >
        <motion.div
          className={`h-full relative ${franchise === "pokemon" ? "rounded-full" : franchise === "overwatch" ? "rounded-[2px]" : "rounded-sm"}`}
        ...
        >
          ...
          {franchise === "overwatch" && (
            <div
              className="absolute inset-0"
              style={{ backgroundImage: `linear-gradient(90deg, transparent 60%, color-mix(in srgb, ${color} 80%, white) 100%)` }}
              aria-hidden="true"
            />
          )}
        </motion.div>
...
function SkillCategory({ title, skillList, startIndex = 0, franchise }: { title: string; skillList: { name: string; level: number }[]; startIndex?: number; franchise: Franchise }) {
  return (
    <div className={`pixel-border p-4 sm:p-5 backdrop-blur-sm ${franchise === "pokemon" ? "rounded-xl" : franchise === "overwatch" ? "rounded-[3px]" : "rounded-lg"}`} style={{ backgroundColor: "color-mix(in srgb, var(--t-bg-card) 60%, transparent)" }}>
      <div className="flex items-center gap-2 mb-4">
        <div className={`w-2 h-2 ${franchise === "pokemon" ? "rounded-full" : "rounded-sm"}`} style={{ backgroundColor: "var(--t-accent2)" }} aria-hidden="true" />
      ...
      <div className="flex flex-col gap-2.5" role="list" aria-label={title}>
        {skillList.map((skill, i) => (
          <HPBar key={skill.name} name={skill.name} level={skill.level} index={startIndex + i} franchise={franchise} />
        ))}
      </div>
    </div>
  );
    ...
    <section id="skills" className="py-12" aria-label="Skills">
      <div className="container max-w-5xl mx-auto px-4 sm:px-6 md:px-4">
        ...
        <div className="space-y-5">
          <SkillCategory title="Programming & Development" skillList={skills.programmingAndDevelopment} franchise={config.franchise} />
          <SkillCategory title="Operating Systems" skillList={skills.operatingSystems} startIndex={skills.programmingAndDevelopment.length} franchise={config.franchise} />
          <SkillCategory title="Languages" skillList={skills.languages} startIndex={skills.programmingAndDevelopment.length + skills.operatingSystems.length} franchise={config.franchise} />
        </div>
